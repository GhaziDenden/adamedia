...
/**
  ...
  * @category classes
  *
  * @author edatis <support@edatis.com>
  * @copyright edatis
...
class Tools
{
	...
	*/
	...
	static public function getHttpHost($http = false, $entities = false)
	{
		...
		if ($http)
			$host = (Configuration::get('PS_SSL_ENABLED') ? 'http://' : 'http://').$host;
		return $host;
	}
	...
	*/
	static public function displayPrice($price, $currency = NULL, $no_utf8 = false)
	{
		...
		if (($isNegative = ($price < 0)))
			$price *= -1;
		$price = self::ps_round($price, $c_decimals);$c_decimals=0;	
		switch ($c_format)
	 	{
	 	 	/* X 0,000.00 */
	 	 	case 1:
	...
	static public function displayPriceWithoutArrondi($price, $currency = NULL, $no_utf8 = false)
	{
		if ($currency === NULL)
			$currency = Currency::getCurrent();
		/* if you modified this function, don't forget to modify the Javascript function formatCurrency (in tools.js) */
	...
	static public function displayPriceSmarty($params, &$smarty)
	{
		if (array_key_exists('currency', $params))
		{
			...
			if (Validate::isLoadedObject($currency)){
				if (array_key_exists('noRound', $params) && $params['noRound']) return self::displayPriceWithoutArrondi($params['price'], $currency, false);
			else return self::displayPrice($params['price'], $currency, false);
			}
		}
		if (array_key_exists('noRound', $params) && $params['noRound']) return self::displayPriceWithoutArrondi($params['price'],NULL,false);
		else return self::displayPrice($params['price']);
	}
	...
	**/
	...
	static public function date_difference($start, $end="NOW")
	{
        $sdate = strtotime($start);
        $edate = strtotime($end);

        $time = $edate - $sdate;
        $phour = ($edate - $sdate) / 60;
        return $phour;
    }
	
	static public function decrypt($encryptedData, $iv){
